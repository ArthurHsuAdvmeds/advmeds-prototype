...
**只有兩種結果文字，標題需依下表逐字呈現，不可改動：**

| 條件 | 色調 | 標題 |
| --- | --- | --- |
| 低肌肉量 + 低肌力 | 紅 | 符合肌少症診斷 |
| 其餘（含只符合其中一項） | 綠 | 未符合肌少症診斷 |

- 臨床上沒有「可能肌少症（尚未符合完整判定）」這個中間狀態，**不要加回來**。
- 肌肉量（SMI 與 ASM/BMI 皆無值）或握力未填時，尚無結果，整塊結果文字不顯示（`hasResult`）。

### 4.5 結果標籤

評估結果標題旁**只使用以下三種標籤**，依條件各自出現：
...
| ASM / BMI | ratio | — | `門檻 ≥ asmBmiT` | `< asmBmiT` |
